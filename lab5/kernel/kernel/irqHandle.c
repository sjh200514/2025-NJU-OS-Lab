...
void syscallOpen(struct StackFrame *sf) {
    ...
    // 检查 O_DIRECTORY 标志和文件类型是否匹配
    if((destInode.type != DIRECTORY_TYPE && (sf->edx >> 3) % 2 == 1) || (destInode.type == DIRECTORY_TYPE && (sf->edx >> 3) % 2 == 0))
    {
...
void syscallWriteFile(struct StackFrame *sf) {
  ...
  while(used < size)
  {
    ...
    if(quotient >= inode.blockCount)
    {
      ...
      if(ret == -1)
      {
        if(file[sf->ecx - MAX_DEV_NUM].offset + used > inode.size)
        {
          inode.size = file[sf->ecx - MAX_DEV_NUM].offset + used;
        }
        size = used;
        break;
      }
    ...
    // 当前块足够容纳剩余数据
    if(sBlock.blockSize - start >= size - used)
    {
      for(int k = 0; k < size - used; k++)
      {
        buffer[j++] = str[i++];
      }
...
void syscallReadFile(struct StackFrame *sf) {
  ...
  {
    readBlock(&sBlock,&inode,quotient,buffer);
    int rest = sBlock.blockSize - start;
    // 处理文件末尾
    if(rest + file[sf->ecx - MAX_DEV_NUM].offset >= inode.size)
    {
