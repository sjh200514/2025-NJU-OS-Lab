...
void timerHandle(struct StackFrame *sf)
{
  ...
  if(pcb[current].timeCount >= MAX_TIME_COUNT)
  {
    ...
    int next = current;
    for(int j = (current + 1) % MAX_PCB_NUM; j != current; j = (j + 1) % MAX_PCB_NUM)
    {
...
void syscallFork(struct StackFrame *sf)
{
  ...
  disableInterrupt();
  pcb[i].stackTop = pcb[current].stackTop - (uint32_t)&(pcb[current]) + (uint32_t)&(pcb[i]);
  pcb[i].prevStackTop = pcb[current].prevStackTop - (uint32_t)&(pcb[current]) + (uint32_t)&(pcb[i]);
  pcb[i].state = STATE_RUNNABLE;
  pcb[i].timeCount = 0;
  pcb[i].sleepTime = 0;
  pcb[i].pid = i;
