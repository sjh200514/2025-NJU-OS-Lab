...
void syscallReadStdIn(struct StackFrame *sf) {
  ...
  if(dev[STD_IN].value == 0)
  {
    ...
    asm volatile("movb $0, %%es:(%0)"::"r"(str + i)); // 添加字符串结束符
    pcb[current].regs.eax = i; // 返回读取的字节数
    return; 
  }
  // 如果已有进程阻塞,直接返回失败
...
void syscallSemPost(struct StackFrame *sf) {
  ...
  // V操作：信号量值加1
  sem[i].value++;
  // 若value取值不⼤于0,则释放⼀个阻塞在该信号量上进程(即将该进程设置为就绪态)
  if(sem[i].value <= 0)
  {
...
void syscallSemDestroy(struct StackFrame *sf) {
  ...
  int i = sf->edx;
  if(i < 0 || i >= MAX_SEM_NUM || sem[i].state == 0 || sem[i].pcb.next != &sem[i].pcb)
  {
    pcb[current].regs.eax = -1;
    return;
  }
